    ...
    <tr key={item.id} className="border-b border-slate-100 hover:bg-blue-50/30 transition">
      ...
      <td className="px-4 py-3">
        <div className="flex items-center justify-center gap-1">
          {userRole !== 'viewer' && (
            <button onClick={() => setShowDaily(item)}
              className="p-1.5 rounded hover:bg-slate-100 text-slate-400 hover:text-blue-600 transition" title="棚卸履歴">
          ...
          {userRole !== 'viewer' && (
            <button onClick={() => { setEditItem(item); setShowForm(true) }}
              className="p-1.5 rounded hover:bg-slate-100 text-slate-400 hover:text-amber-600 transition" title="編集">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {userRole === 'admin' && (
            <button onClick={() => handleDelete(item)}
              className="p-1.5 rounded hover:bg-slate-100 text-slate-400 hover:text-red-600 transition" title="削除">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {userRole === 'editor' && (
            isPendingDelete(item.id) ? (
              ...
              <button onClick={() => handleRequestDelete(item)}
                className="p-1.5 rounded hover:bg-slate-100 text-slate-400 hover:text-red-600 transition" title="削除を申請">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                </svg>
              </button>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">
            {location?.name}
            {selectedCategory && (
              <span className="text-lg font-normal text-blue-600 ml-2">- {selectedCategory}</span>
            )}
          </h1>
