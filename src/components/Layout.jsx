                ...
                >
                  ...
                  {contextMenuId === loc.id && editingId !== loc.id && (
                    <div className="absolute right-0 top-full z-20 bg-white border border-slate-200 rounded-lg shadow-lg py-1 min-w-36" onClick={(e) => e.stopPropagation()}>
                      {userRole === 'admin' && (
                        <>
                          <button onClick={(e) => { e.preventDefault(); setContextMenuId(null); setEditingId(loc.id); setEditName(loc.name) }}
                          ...
                          <button onClick={(e) => { e.preventDefault(); setContextMenuId(null); handleArchive(loc.id, loc.archived) }}
                            className="w-full flex items-center gap-2 px-3 py-1.5 text-xs text-slate-600 hover:bg-slate-50">
                          ...
                          <button onClick={(e) => { e.preventDefault(); setContextMenuId(null); handleDelete(loc.id) }}
                      ...
                      {userRole !== 'admin' && (
                        <>
                          <button onClick={(e) => { e.preventDefault(); setContextMenuId(null); handleRequestRename(loc.id, loc.name) }}
                          ...
                          {pendingRequestForLocation(loc.id) ? (
                            ...
                            <button onClick={(e) => { e.preventDefault(); setContextMenuId(null); handleRequestDeletion(loc.id) }}
            ...
            {userRole === 'admin' && archivedLocations.length > 0 && (
              <div className="mt-3">
                ...
                {archivedLocations.map((loc) => (
                  <div key={loc.id} className="relative flex items-center rounded text-sm text-slate-400 italic"
                    onContextMenu={(e) => { e.preventDefault(); setContextMenuId(contextMenuId === loc.id ? null : loc.id) }}>
                    <Link to={`/location/${loc.id}`} className="flex-1 block px-3 py-1.5 truncate">{loc.name}</Link>
                    {contextMenuId === loc.id && (
                      <div className="absolute right-0 top-full z-20 bg-white border border-slate-200 rounded-lg shadow-lg py-1 min-w-36" onClick={(e) => e.stopPropagation()}>
                        <button onClick={() => { setContextMenuId(null); handleArchive(loc.id, loc.archived) }}
                        ...
                        <div className="border-t border-slate-100 my-1" />
                        <button onClick={() => { setContextMenuId(null); handleDelete(loc.id) }}
                          className={`w-full flex items-center gap-2 px-3 py-1.5 text-xs ${deleteConfirm === loc.id ? 'text-red-600 font-semibold' : 'text-red-500 hover:bg-red-50'}`}>
                          <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
          ...
          <div className="border-t border-slate-200 p-2">
            {showAddForm ? (
              <div className="flex gap-1">
                ...
                <button onClick={() => { setShowAddForm(false); setNewLocationName('') }}
            ...
            {userRole === 'admin' && (
              <>
                ...
                {deletionRequests.length > 0 && (
                  <button onClick={() => setShowApprovalPanel(!showApprovalPanel)}
              ...
              return (
                <div key={req.id} className="px-4 py-2 border-b border-slate-50 last:border-b-0">
                  ...
                  <div className="flex gap-2 mt-1.5">
                    <button onClick={() => handleApproveDeletion(req.id, req.location_id)}
                      className="px-2 py-0.5 text-xs bg-red-500 text-white rounded hover:bg-red-600">
                      承認（削除）
                    </button>
                    <button onClick={() => handleRejectDeletion(req.id)}
